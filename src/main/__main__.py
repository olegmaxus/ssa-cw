...
dates, F = aux.read_graph_data_snp500(sys.argv[1])

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ # ~ REQUESTING TO PLOT PARTIALS

# asking to plot the parts of the huge TS and their mip SSA-derived components.
full_partial = str(input("\nHello!\nPlease, specify:\nShould the program analyse the time series partially or fully?\n"
                         "WARNING: full analysis may not be suitable for all computers, as requires high computing power.\n"
                         "Please, peint: FULL or PARTIAL: "))
...
if full_partial == "FULL":
    ...
    full_partial = False
    print("\nUnrecognizable input, assuming: partial analysis.\n")
...
''' Performing SVD for full data set '''

x = np.arange(len(F))

if full_partial:
    elementary_span, x = pvp.full_svd_process_and_plot_snp500(dates, F, "1926 - 2018", plot=other_yn)
    ...
    f_overall_no_noise = f_predicted_tr + f_predicted_p1 + f_predicted_p2 + f_predicted_p3
    ...
    which_scale = str(input("\nThe scale should be log/linear/symlog/logit?\nPlease, specify: "))
    listed = ["log", "linear", "symlog", "logit"]

    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ # ~ PLOTTING FULL TIME SERIES AND COMPONENTS
    estimated = [(F, r"$\mathcal{F}(x)$", '', true_false[0]),
                 (f_predicted_tr,    r"$\tilde{F}^{(\mathrm{trend})}$", 'orange', true_false[1]),
                 (f_predicted_p1, r"$\tilde{F}^{(\mathrm{period})_1}$", '', true_false[2]),
                 (f_predicted_p2, r"$\tilde{F}^{(\mathrm{period})_2}$", '', true_false[3]),
                 (f_predicted_p3, r"$\tilde{F}^{(\mathrm{period})_3}$", '', true_false[4]),
                 (f_predicted_noise,    r"$\tilde{\eta}(x)$", 'black', true_false[5]),
                 (f_overall_no_noise,       r"$\tilde{F}(x)$", 'red', true_false[6])]

    if which_scale in listed:
        pvp.plot_full_snp500(dates, estimated, which_scale)
    else:
        pvp.plot_full_snp500(dates, estimated)
else:
    ...
    F_1, dates_1 = F[:300], dates[:300]
    F_2, dates_2 = F[300:600], dates[300:600]
    F_3, dates_3 = F[600:900], dates[600:900]
    F_4, dates_4 = F[900:], dates[900:]

    elementary_span_1, x_1 = pvp.full_svd_process_and_plot_snp500(dates_1, F_1, "1926 - 1951", 1, plot=other_yn)
    elementary_span_2, x_2 = pvp.full_svd_process_and_plot_snp500(dates_2, F_2, "1951 - 1976", 2, plot=other_yn)
    elementary_span_3, x_3 = pvp.full_svd_process_and_plot_snp500(dates_3, F_3, "1976 - 2001", 3, plot=other_yn)
    elementary_span_4, x_4 = pvp.full_svd_process_and_plot_snp500(dates_4, F_4, "2001 - 2018*", 4, plot=other_yn)
    ...
    # plotting #
    if string_yn:
        pvp.plot_ts_and_components(dates_1, collection_ts_1, collection_noisy_1, "1926 - 1951", 1)
    ...
    # plotting #
    if string_yn:
        pvp.plot_ts_and_components(dates_2, collection_ts_2, collection_noisy_2, "1951 - 1976", 2)
    ...
    # plotting #
    if string_yn:
        pvp.plot_ts_and_components(dates_3, collection_ts_3, collection_noisy_3, "1976 - 2001", 3)
    ...
    # plotting #
    if string_yn:
        pvp.plot_ts_and_components(dates_4, collection_ts_4, collection_noisy_4, "2001 - 2018*", 4)
